...
static inline void pg_toggle_wb_journal(struct zdm *znd, struct map_pg *expg)
{
	...
	/* migrate journal from 1 -> 2 -> home -> 1 ... */
	...
	if (re_cache && (expg->lba != expg->last_write)) {
		if (!expg->data.addr) {
...
static u64 journal_alloc(struct zdm *znd, u32 nblks, u32 *num)
{
	...
	do {
		...
		Z_DBG(znd, "WB Journal full. Wrapping ... ");
		next = 0;
...
out_unlock:
	...
	if (tlba == 0 && retry-- > 0)
		Z_ERR(znd, "MD Journal full .. needs purge");
